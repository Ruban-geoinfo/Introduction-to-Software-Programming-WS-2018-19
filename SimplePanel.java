...
public class SimplePanel extends JPanel{
	...
	public void paintComponent(Graphics g) {
		...
		//** To Change the drawer and their attributes **//
		...
		for(Circle c:content.getCircles()){
			...
			Ellipse2D.Double shape = new Ellipse2D.Double(c.getCoordinates().getX()-(c.getDiameter()/2) ,c.getCoordinates().getY()-(c.getDiameter()/2), c.getDiameter(), c.getDiameter() );
		    g2d.draw(shape);
		...
		for(Rectangle r: content.getRectangles()){
			...
			Rectangle2D.Double shape = new Rectangle2D.Double(r.getCoordinates().getX(), r.getCoordinates().getY(), r.getWidth(), r.getHeight());		  
			g2d.draw(shape);
		...
		//** DRAW POLYGONS**//   not to be considered for this task
...
//		}
		...
		//** Here define DRAW LABELS**//
		
		for(Label l: content.getLabels()){
			g2d.setColor( l.getStrokeColor());
			g2d.drawString(l.getText(), Math.round(l.getPosition().getX())-12 , Math.round(l.getPosition().getY())-2 );
		}
